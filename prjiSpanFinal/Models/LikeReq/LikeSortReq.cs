...
namespace prjiSpanFinal.Models.LikeReq
{
    public class LikeSortReq
    {
        ...
        {
            iSpanProjectContext db = new iSpanProjectContext();
            var LikeProduct = new List<Product>();
            ...
            List<MyLikeShowItem> list = (new MyLikeFactory()).toShowItem(LikeProduct);
            ...
            if (!String.IsNullOrWhiteSpace(keyword))
            {
                ...
                for (int i = 0; i < keys.Length; i++)
                {
                    list = list.Where(a => a.Like.Product.ProductName.Contains(keys[i]) || a.ProductDetail.UnitPrice.ToString().Contains(keys[i])
                    || a.ProductDetail.Quantity.ToString().Contains(keys[i])).ToList();
                }
                ...
                //    case 1:
                //        //全部按讚
                //        list = (new MyLikeFactory()).SearchItem(list);
                //        break;
                //    case 3:
                //        //熱銷排序
                //        list = ((new MyLikeFactory()).SearchItem(list)).OrderByDescending(s => s.salesVolume).ToList();
                //        break;
                //    case 4:
                //        //價高排序
